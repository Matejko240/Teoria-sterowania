...
% ZADANIE 1
% symulacja i kreslenie wykresow dla dynamiki satelity (sD)
[t1,y1]=ode45(sD,[0 T],x0);
figure(2);
subplot(2,1,1);
plot(t1,y1)
title("Dynamika satelity sD")
xlabel("t [s]")
ylabel("x");
% symulacja i kreslenie wykresow dla aproksymacji liniowej dynamiki satelity (sLD)
[tl1, yl1]=ode45(sLD,[0 T],x0L);
subplot(2,1,2);
plot(tl1,yl1);
title("Liniowa dynamika satelity");
xlabel("t [s]");
ylabel("x");
...
% ZADANIE 3
% tworzenie kodu dla dynamiki satelity (sD)
sD=@(t,x) [x(2);-k./(x(1)^2)+x(1)*(omega+x(4))^2+1/m*(ur(t)+dr(t));x(4);0];
% tworzenie kodu dla aproksymacji liniowej dynamiki satelity (sLD)
sLD=@(t,x) A*x+B*dr(t);

% symulacja i kreslenie wykresów dla dynamiki satelity (sD)
[t3,y3]=ode45(sD,[0 T],x0);
figure(4)
subplot(2, 1, 1)
plot(t3,y3)
title("Dynamika satelity sD")
xlabel("t [s]"); 
ylabel("x");
% symulacja i kreslenie wykresów dla aproksymacji liniowej dynamiki satelity (sLD)
[tl3,yl3]=ode45(sLD,[0 T],x0L);
subplot(2,1,2)
plot(tl3,yl3)
title("Aporksymacja liniowej dynamiki satelity sD")
xlabel("t [s]"); 
ylabel("x");
% 3
% radialne zakłócenie o charakterze okresowym z amplitudą równą 5%
% grawitacyjnego przyciągania satelity przez Ziemię o okresie równym okresowi
% obrotu Ziemi

% warunki początkowe i radialne zakłócenie
...
ylabel("x");
% symulacja i kreslenie wykresów dla aproksymacji liniowej dynamiki satelity (sLD)
[tl4,yl4]=ode45(sLD,[0 T],x0L);
subplot(2,1,2)
plot(tl4,yl4)
title("Aproksymacja liniowej dynamiki satelity sLD")
xlabel("t [s]");
ylabel("x");
